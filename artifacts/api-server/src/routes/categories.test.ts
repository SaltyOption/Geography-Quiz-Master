...
const ADMIN_ID = "user_category_admin";
...
describe("category endpoints reject a non-numeric id with 400", () => {
  it("PATCH /api/categories/:id 400s a non-numeric id without reading or writing the DB", async () => {
    pushDbResult([{ id: 1 }]);

    const res = await asAdmin(
      request(app).patch("/api/categories/abc").send({ name: "Renamed" }),
    );
  ...
  it("DELETE /api/categories/:id 400s a non-numeric id without deleting from the DB", async () => {
    pushDbResult([{ id: 1 }]);

    const res = await asAdmin(request(app).delete("/api/categories/abc"));
    expect(res.status).toBe(400);
    expect(dbResultQueue).toHaveLength(1); // queued result was never consumed
  });
});
