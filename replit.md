...
- **Active alerting (optional):** when broken images are found and `BROKEN_IMAGE_ALERT_WEBHOOK_URL` is set, the script POSTs a JSON notification (Slack-compatible `text` summary plus a structured `broken` array of `{ source, id, url, reason }`) so the team is alerted without watching the Deployments pane. The webhook works with Slack incoming webhooks and generic JSON webhooks. Only genuinely broken images trigger an alert — transient failures (timeouts/DNS/5xx/429) never notify. Alerting is best-effort: when the env var is unset it is a no-op, and a delivery failure is logged but never changes the run's exit code. Set `BROKEN_IMAGE_ALERT_WEBHOOK_URL` in the production (and/or dev) Secrets pane.
